<script lang="ts">
	...
	const setupMap = (node: HTMLElement) => {
		...
		map.getView().fit([-14288085.579078684, 2613236.080750007, -7307971.125095623, 6578328.874748718]);
...
<div id="layout">
	<div id="about">
		...
		<p>The <a href="https://www.census.gov/geographies/mapping-files/time-series/geo/carto-boundary-file.html">US States
			shapefile</a> comes from the United States Census Bureau.</p>
		...
		<table>
			<tbody>
			<tr>
				<td><label><input type="checkbox" bind:checked={costOfLivingEnabled} onchange={() => {
		toggleLayer('Cost of Living', costOfLivingEnabled);
	}}> Cost of Living</label></td>
				<td><a href="https://worldpopulationreview.com/state-rankings/cost-of-living-index-by-state">Source</a></td>
			</tr>
			<tr>
				<td><label><input type="checkbox" bind:checked={educationEnabled} onchange={() => {
		toggleLayer('Education', educationEnabled);
	}}> Public School Rankings</label></td>
				<td><a href="https://worldpopulationreview.com/state-rankings/public-school-rankings-by-state">Source</a></td>
			</tr>
			<tr>
				<td><label><input type="checkbox" bind:checked={fiberEnabled} onchange={() => {
		toggleLayer('Fiber', fiberEnabled);
	}}> Fiber Internet Rank</label></td>
				<td><a href="https://broadbandnow.com/report/fiber-optic-availability-map">Source</a></td>
			</tr>
			<tr>
				<td>
					<label><input type="checkbox" bind:checked={harrisVotesEnabled} onchange={() => {
		toggleLayer('Harris Votes', harrisVotesEnabled);
			...
			<tr>
				<td><label><input type="checkbox" bind:checked={homicidesEnabled} onchange={() => {
		toggleLayer('Homicides', homicidesEnabled);
	}}> Homicides per 100k</label></td>
				<td><a href="https://worldpopulationreview.com/state-rankings/murder-rate-by-state">Source</a></td>
			</tr>
			<tr>
				<td><label><input type="checkbox" bind:checked={marijuanaEnabled} onchange={() => {
		toggleLayer('Marijuana', marijuanaEnabled);
	}}> Marijuana Legality Status</label></td>
